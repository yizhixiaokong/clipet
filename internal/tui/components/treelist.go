...
// buildConnectorPrefixStyled builds connector prefix with connector style applied
func (t TreeList) buildConnectorPrefixStyled(node *TreeNode, baseStyle lipgloss.Style) string {
	raw := t.buildConnectorPrefixRaw(node)
	if raw == "" {
		return ""
	}
	return baseStyle.Render(raw)
}

// buildConnectorPrefix generates tree connector lines
func (t TreeList) buildConnectorPrefix(node *TreeNode) string {
	if node.depth == 0 {
		return ""
	}
	...
	current := node
	var ancestors []bool // true = is last child
	visited := make(map[*TreeNode]bool) // Prevent infinite loops
	...
	for i, isLast := range ancestors {
		if i == len(ancestors)-1 {
			...
			if isLast {
				prefix.WriteString(t.Styles.Connector.Render("└── "))
			} else {
				prefix.WriteString(t.Styles.Connector.Render("├── "))
			}
		} else {
			// Ancestor levels: show vertical line or space
			if isLast {
				prefix.WriteString("    ")
			} else {
				prefix.WriteString(t.Styles.Connector.Render("│   "))
